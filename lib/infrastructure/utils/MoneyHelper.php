<?php
class Calendarista_MoneyHelper{
	...
	public static function getCurrencySymbol(){
		$generalSetting = Calendarista_GeneralSettingHelper::get();
		return self::toCurrencySymbol($generalSetting->currency);
	}
	public static function percentage($percentage, $amount)
	{
		return ($amount / 100) * $percentage;
	}
	public static function toDouble($val, $precision = 2)
	{
		$generalSetting = Calendarista_GeneralSettingHelper::get();
		...
		*/
		return number_format(floatval($val), $precision, $generalSetting->decimalPoint, $generalSetting->thousandSep);
	}
	public static function getCurrencySymbolPosition($currency = null){
		$generalSetting = Calendarista_GeneralSettingHelper::get();
		if($generalSetting->currencySymbolPlacement !== -1){
			return $generalSetting->currencySymbolPlacement;
		}
		if($currency === null){
			$currency = $generalSetting->currency;
		}
	...
	public static function getCurrencyPrecision($precision = 2){
		$generalSetting = Calendarista_GeneralSettingHelper::get();
		$currency = $generalSetting->currency;
		$currencyDesc = self::getCurrencyByCode($currency);
		return $currencyDesc ? $currencyDesc['decimal_digits'] : $precision;
	}
	public static function toCurrencySymbol($code){
		$currencyDesc = self::getCurrencyByCode($code);
		return $currencyDesc['symbol_native'];
	}
	public static function toLongCurrency($value, $toDouble = true){
		$result = self::formatCurrencySymbol($toDouble ? self::toDouble($value) : $value, false);
		return $result;
	}
	public static function toShortCurrency($value, $toDouble = true){
		$result = self::formatCurrencySymbol($toDouble ? self::toDouble($value) : $value, true);
		return $result;
	}
	public static function formatCurrencySymbol($value, $shortFormat = false, $currency = null, $symbol = null){
		if(!$currency || !$symbol){
			$generalSetting = Calendarista_GeneralSettingHelper::get();
			$currency = $generalSetting->currency;
			$symbol = self::toCurrencySymbol($currency);
		}
		$pos = self::getCurrencySymbolPosition($currency);
		if($pos === 0/*left*/){
			if($shortFormat || $symbol === $currency){
	...
	public static function getCurrencyByCode($code){
		...
		$result = json_decode($currencies, true, 3000);
